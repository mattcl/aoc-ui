<script lang="ts">
  ...
  function makeData(raw: Summary[], days: number[]) {
    ...
    let totals = raw.map((x) => {
      return days.reduce((acc, day) => {
        let key = `day_${day}`;
        let val = x[key] === null ? 0 : x[key];
        return acc + val;
      }, 0);
    });

    days.map((day) => {
      let dayData = dataForDay(raw, day, totals);
  ...
  function dataForDay(raw: Summary[], day: number, totals: number[]) {
    return raw.map((x, i) => {
      let key = `day_${day}`;
      let val = x[key];
      return val === null || totals[i] == 0 ? 0 : (val / totals[i]) * 100 ;
    });
  }
</script>
